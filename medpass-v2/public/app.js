...
function renderDocGrid(docs) {
  ...
  docGrid.querySelectorAll('.btn-doc-chat').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (!doc) return;
      // Send doc text to chat as context
...
async function processAndStoreDoc(file) {
  ...
  try {
    ...
    // 3. Process (Solve All in background)
    docProcessSub.textContent = 'Generating questions & answers…';
    const runId = await runSolveAllForDoc(text, file.name, (pct) => {
      docProcessFill.style.width = `${55 + pct * 0.40}%`;
    ...
    if (docId) {
      await fetch(`/api/study/documents/${docId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: 'done', runId }),
      });
    }
...
async function runSolveAllForDoc(text, fileName, onProgress) {
  ...
  const saveRes = await fetch('/api/study/runs', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ documentName: fileName, items: runItems }),
  });
  const saveData = await saveRes.json();
...
function appendMessage(role, text, isError, attachmentNames) {
  ...
  if (role !== 'user') {
    ...
    meta.textContent = isError ? 'Error' : 'MedPass Tutor';
...
function appendTyping() {
  ...
  wrap.innerHTML = '<div class="msg-meta">MedPass Tutor</div><span class="typing-dots"><span></span><span></span><span></span></span>';
  chatLog.appendChild(wrap);
  chatLog.scrollTop = chatLog.scrollHeight;
  return wrap;
}
 ...
 */
async function saveStudyRun(attachmentName, items) {
  try {
    const res = await fetch('/api/study/runs', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ documentName: attachmentName || 'Untitled', items }),
    });
    if (!res.ok) {

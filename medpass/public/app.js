...
async function loadDashboardView() {
  try {
    ...
    if (achieveRes.ok) {
      ...
      const nextEl = document.getElementById('dashNextAchievement');
      if (nextEl) nextEl.innerHTML = `<div class="dash-panel-empty">Complete quizzes to earn achievements 🏆</div>`;
    }
...
const dashQaQuickQuiz = document.getElementById('dashQaQuickQuiz');
if (dashQaQuickQuiz) dashQaQuickQuiz.onclick = () => {
  const topic = prompt('Enter a topic for your Quick Quiz\n(e.g. "cardiology", "pharmacology", or leave blank for general medicine):');
  if (topic === null) return;
  const t = topic.trim() || 'general clinical medicine';
  openAssessSetup({ type: 'topic', topic: t }, '⚡ Quick Quiz: ' + t);
};
...
function renderFlashcards(cards) {
  ...
  // Always hide the review summary whenever we render the deck/empty state —
  // these three panels are mutually exclusive and should never show together.
  if (flashReviewSummary) flashReviewSummary.hidden = true;
  if (!cards.length) {

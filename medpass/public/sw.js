...
// App-shell files (any top-level HTML/CSS/JS page) change as the app is
// developed and must always be checked against the network first, so users
// get updates on a normal reload with no manual cache-busting required.
// Only /vendor/ files (and other non-html/css/js static assets) are safe
// to treat as cache-first, since those don't change without a filename change.
...
self.addEventListener('fetch', (event) => {
  ...
  // For navigation requests: network first, fall back to the same cached page

...
/** GET /api/study/documents/:id/text — return text_content for quick-quiz generation */
function getDocumentText(req, res, id, session) {
  const doc = q.getDocumentById(id);
  if (!doc || doc.user_id !== session.user.id) {
    return sendJson(res, 404, { error: 'Document not found.' });
  }
  if (doc.status !== 'done') {
    return sendJson(res, 400, { error: 'Document is not ready yet.' });
  }
...
function getChatHistory(req, res, session, url) {
  const limit = Number(url.searchParams.get('limit')) || 100;
  const messages = q.listChatMessages(session.user.id, limit);
  return sendJson(res, 200, { messages });
}

/** POST /api/study/chat-history — save a batch of new messages */
function saveChatHistory(req, res, body, session) {
  ...
  try {
    q.saveChatMessages(session.user.id, messages);
    return sendJson(res, 200, { saved: messages.length });
  } catch (err) {
    console.error('saveChatHistory error:', err.message);
    return sendJson(res, 500, { error: 'Could not save chat history.' });
  }
}

/** DELETE /api/study/chat-history — wipe all messages for this user */
function deleteChatHistory(req, res, session) {
  q.clearChatMessages(session.user.id);
  return sendJson(res, 200, { cleared: true });
}

/** GET /api/study/notes */
function listNotes(req, res, session) {
  const notes = q.listNotesForUser(session.user.id);
  return sendJson(res, 200, { notes });
}

/** POST /api/study/notes */
function createNoteHandler(req, res, body, session) {
  const { title, body: noteBody, color } = body || {};
  const result = q.createNote(session.user.id, { title, body: noteBody, color });
  const note = q.getNoteById(result.lastInsertRowid, session.user.id);
  return sendJson(res, 201, { note });
}

/** PATCH /api/study/notes/:id */
function updateNoteHandler(req, res, id, body, session) {
  const note = q.updateNote(id, session.user.id, body || {});
  if (!note) return sendJson(res, 404, { error: 'Note not found.' });
  return sendJson(res, 200, { note });
}

/** DELETE /api/study/notes/:id */
function deleteNoteHandler(req, res, id, session) {
  q.deleteNote(id, session.user.id);
  return sendJson(res, 200, { deleted: true });
}

/** GET /api/study/focus-sessions */
function getFocusStats(req, res, session) {
  const stats = q.getFocusStatsForUser(session.user.id);
  return sendJson(res, 200, stats);
}

/** POST /api/study/focus-sessions */
function logFocusSessionHandler(req, res, body, session) {
  const { durationMins, subject, completed } = body || {};
  q.logFocusSession(session.user.id, { durationMins, subject, completed });
  return sendJson(res, 201, { logged: true });
}

/** GET /api/study/achievements */
function getAchievementsHandler(req, res, session) {
  const data = q.getAchievementsForUser(session.user.id);
  return sendJson(res, 200, data);
}

/** GET /api/study/weak-spots */
function getWeakSpotsHandler(req, res, session) {
  const weakSpots = q.getWeakSpotsForUser(session.user.id);
  return sendJson(res, 200, { weakSpots });
}
...
module.exports = {
  ...
  getAnalytics,
  search,
  getChatHistory, saveChatHistory, deleteChatHistory,
  listNotes, createNoteHandler, updateNoteHandler, deleteNoteHandler,
  getFocusStats, logFocusSessionHandler,
  getAchievementsHandler, getWeakSpotsHandler,
};

...
const ALLOWED_AUDIO = new Set(['audio/mpeg','audio/mp3','audio/mp4','audio/x-m4a',
  'audio/m4a','audio/wav','audio/wave','audio/webm','audio/ogg','video/mp4',
  'video/webm','audio/x-wav','application/octet-stream']);

const transcribeLimiter = rateLimiter({ windowMs: 60_000, max: 10 });
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      ...
      if (size > MAX_AUDIO_BYTES) {
        reject(Object.assign(new Error('Audio file too large (max 25 MB).'), { statusCode: 413 }));
        req.destroy();
        return;
      }
      chunks.push(chunk);
...
function parseMultipartFile(body, boundary) {
  // Use latin1 to safely convert binary to string for searching
  const raw = body.toString('latin1');
  const sep = '--' + boundary;

  // Find the part with name="audio"
  let searchName = 'name="audio"';
  let idx = raw.indexOf(searchName);
  if (idx === -1) { searchName = 'name="file"'; idx = raw.indexOf(searchName); }
  if (idx === -1) return null;

  // Find the blank line separating headers from data
  const headerEnd = raw.indexOf('\r\n\r\n', idx);
  if (headerEnd === -1) return null;

  // Extract headers to get filename and content-type
  const headerBlock = raw.slice(idx, headerEnd);
  ...
  return {
    ...
    mimeType: (ctMatch?.[1] || 'audio/mpeg').trim(),
  };
}

function buildWhisperForm(audioBuffer, filename, mimeType) {
  const boundary = '----GroqBoundary' + crypto.randomBytes(8).toString('hex');
  const CRLF = '\r\n';
  const parts = Buffer.concat([
    Buffer.from(`--${boundary}${CRLF}Content-Disposition: form-data; name="model"${CRLF}${CRLF}whisper-large-v3${CRLF}`, 'utf8'),
    Buffer.from(`--${boundary}${CRLF}Content-Disposition: form-data; name="response_format"${CRLF}${CRLF}json${CRLF}`, 'utf8'),
    Buffer.from(`--${boundary}${CRLF}Content-Disposition: form-data; name="audio"; filename="${filename}"${CRLF}Content-Type: ${mimeType}${CRLF}${CRLF}`, 'utf8'),
    audioBuffer,
...
async function transcribe(req, res, session) {
  const userId = session?.user?.id;
  const { allowed, retryAfterMs } = transcribeLimiter(req, 'transcribe', userId);
  if (!allowed) {
    const s = Math.ceil(retryAfterMs / 1000);
    return sendJson(res, 429, { error: `Rate limit hit. Wait ${s}s.`, retryAfter: s });
  }
  ...
  const apiKey = process.env.GROQ_API_KEY;
  if (!apiKey) return sendJson(res, 503, { error: 'AI not configured. Set GROQ_API_KEY.' });
  ...
  catch (err) { return sendJson(res, err.statusCode || 400, { error: err.message }); }

  const file = parseMultipartFile(rawBody, boundary);
  if (!file || !file.buffer.length)
    return sendJson(res, 400, { error: 'No audio file found in request.' });
  if (file.buffer.length > MAX_AUDIO_BYTES)
    return sendJson(res, 413, { error: 'Audio file too large (max 25 MB).' });

  const { body: formBody, contentType } = buildWhisperForm(file.buffer, file.filename, file.mimeType);
  ...
  try {
    ...
    if (!text) return sendJson(res, 502, { error: 'No transcript returned from Whisper.' });

    return sendJson(res, 200, { transcript: text, duration: data?.duration || null });
  } catch (err) {
    if (err.name === 'AbortError') return sendJson(res, 504, { error: 'Transcription timed out. Try a shorter file.' });
    console.error('Whisper fetch failed:', err.message);
    return sendJson(res, 502, { error: 'Could not reach transcription service.' });
  }
}
